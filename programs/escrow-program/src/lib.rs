...
pub mod escrow {
    ...
    pub fn create_escrow(
        ...
        msg!(
            "Escrow created: {} lamports locked for seller {}",
            amount,
            seller
        );
        Ok(())
    ...
    pub fn release_funds(ctx: Context<ReleaseFunds>) -> Result<()> {
        ...
        let seeds: &[&[u8]] = &[b"escrow", buyer_key.as_ref(), &escrow_id_bytes, &[bump]];
        let signer = &[seeds];
...
mod tests {
    ...
    #[test]
    fn escrow_account_size_matches_layout() {
        assert_eq!(EscrowAccount::SIZE, 8 + 32 + 32 + 8 + 1 + 8 + 1);
    }

    #[test]
    fn escrow_status_round_trips_through_anchor_serialization() {
        let encoded = EscrowStatus::Cancelled
            .try_to_vec()
            .expect("status should serialize");
        let decoded = EscrowStatus::try_from_slice(&encoded).expect("status should deserialize");

        assert_eq!(decoded, EscrowStatus::Cancelled);
    }
}
